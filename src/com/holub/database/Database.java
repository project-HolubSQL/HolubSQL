 ...
 * double(l,r)<br>
 * numeric(l,r)</td>
 * <td>floating point, l and r specify the maximum
 * number of digits to the left and right of the decimal.</td>
 * </tr>
 * <tr>
 * <td>char(length)</td>
...
public final class Database { /*
	...
	private final class TableMap implements Map {
		...
		public Object get(Object key) {
			...
			try {
				...
				String message = "Table not created internally and couldn't be loaded."
						+ "(" + e.getMessage() + ")\n";
	 ...
	 * @throws IOException                 Database files couldn't be accessed or
	 *                                     created.
	 * @see #affectedRows()
	 */
	...
	private Table processSelectQuery() throws ParseFailure {
		...
		if (orderingKeys.size() > 0) {
			result.accept(new OrderByVisitor(orderingKeys));
		}
		return result;
	}
	...
	private class ArithmeticExpression implements Expression {
		...
		public Value evaluate(Cursor[] tables) throws ParseFailure {
			...
			return new NumericValue(
					(operator == PLUS) ? (l + r) : (operator == MINUS) ? (l - r) : (operator == TIMES) ? (l * r) :
					/* operator == DIVIDE */ (l / r));
		}
	}
	...
	private class LogicalExpression implements Expression {
		...
		public Value evaluate(Cursor[] tables) throws ParseFailure {
			Value leftValue = left.evaluate(tables);
			Value rightValue = right.evaluate(tables);
			verify(leftValue instanceof BooleanValue
	...
	private class RelationalExpression implements Expression {
		...
		public Value evaluate(Cursor[] tables) throws ParseFailure {
			...
			return new BooleanValue((operator == EQ) ? (l == r)
					: (operator == NE) ? (l != r)
							: (operator == LT) ? (l > r) : (operator == GT) ? (l < r) : (operator == LE) ? (l <= r) :
							/* operator == GE */ (l >= r));
		}
	}
	...
	private class LikeExpression implements Expression {
		...
		public Value evaluate(Cursor[] tables) throws ParseFailure {
			Value leftValue = left.evaluate(tables);
			Value rightValue = right.evaluate(tables);
			verify(leftValue instanceof StringValue
					&& rightValue instanceof StringValue,
